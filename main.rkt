...
(define-struct birdd (x y vy) #:transparent)
...
(define-struct current_state (birdd pipes score variables))
...
(define (get-initial-state)
  (current_state (birdd bird-x bird-initial-y 0)
                 (append '() (list (make-pipe scene-width (+ added-number (random random-threshold)))))
                 0
...
(define (next-state-bird bird gravity)
  (match-let ([(birdd _ y vy) bird])
  (struct-copy birdd bird [y (+ y vy)][vy (+ vy gravity)])))
...
(define (next-state-bird-onspace bird momentum)
  (match-let ([(birdd _ _ vy) bird])
  (struct-copy birdd bird [vy (* -1 momentum)])))
...
(define (change current-state pressed-key)
    (if (key=? pressed-key " ")
        (struct-copy current_state current-state [birdd (next-state-bird-onspace
                                     (get-bird current-state)
                                     (variables-momentum (current_state-variables current-state)))])
          current-state))
...
(define (move-pipes pipes scroll-speed)
  (map (lambda(pipe1) (match-let ([(pipe x _) pipe1]) (struct-copy pipe pipe1 [x (- x scroll-speed)]))) pipes))
...
(define (clean-pipes pipes)
  (filter (lambda (pipe)
            (if ( < (+ (get-pipe-x pipe) pipe-width) 0)
                #f
                #t))
            pipes))
...
(if (> no-pipes (length pipes))
      (append pipes (list (pipe
                           (+ (+ pipe-width pipe-gap) (+ (get-pipe-x (car (reverse pipes))) bird-width))
...
(define (check-pipe-collisions bird pipes)
  ...
  (check-collision-rectangles (make-posn (get-bird-x bird) (get-bird-y bird))
                               ...
                               (make-posn (+ (get-pipe-x (car pipes)) pipe-width) (get-pipe-gap-y (car pipes))))
  
  (check-collision-rectangles (make-posn (get-bird-x bird) (get-bird-y bird))
                              (make-posn (+ (get-bird-x bird) bird-width) (+ (get-bird-y bird) bird-height))
                              (make-posn (get-pipe-x (car pipes)) (+ (get-pipe-gap-y (car pipes)) pipe-self-gap))
                              (make-posn (+ (get-pipe-x (car pipes)) pipe-width) (+ (+ (get-pipe-gap-y (car pipes)) pipe-self-gap) pipe-height)
                                         ))) 
                                                                                                            #t
                                                                                                            (if (null? (cdr pipes))
                                                                                                                #f
...
(define (next-state state)
  (match-let ([(current_state birdd pipes score _) state])
    (struct-copy current_state state [birdd (next-state-bird (get-bird state) initial-gravity)]
...
(define pipe_image (rectangle pipe-width pipe-height "solid" "green"))
...
(define (find-y-up pipe)
  (- (get-pipe-gap-y pipe) (quotient pipe-height 2)))

(define (find-x pipe)
  (+ (get-pipe-x pipe) (quotient pipe-width 2)))

(define (find-y-down pipe)
  (+ (+ (get-pipe-gap-y pipe) pipe-self-gap)(quotient pipe-height 2)))

; Folosind `place-image/place-images` va poziționa pipe-urile pe scenă.
(define (place-pipes pipes scene)
  (if (equal? (length pipes) 1)
      (place-image pipe_image (find-x (car pipes)) (find-y-up (car pipes))
                   (place-image pipe_image (find-x (car pipes)) (find-y-down (car pipes)) scene))
  
      (place-image pipe_image (find-x (car pipes)) (find-y-up (car pipes))
                   (place-image pipe_image (find-x (car pipes)) (find-y-down (car pipes)) (place-pipes (cdr pipes) scene)))))
